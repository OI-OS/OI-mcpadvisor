 ...
 */
const convertToMeilisearchDocuments = (data: GetMcpApiResponse): Record<string, any>[] => {
  return Object.entries(data).map(([id, server]) => ({
    id,
    title: server.display_name,
    description: server.description,
    github_url: server.repository.url,
    categories: Array.isArray(server.categories) ? server.categories.join(',') : '',
 ...
 */
const convertHitToServerResponse = (hit: Record<string, any>): MCPServerResponse => ({
  id: hit.id,
  title: hit.title,
...
export class MeilisearchSearchProvider implements SearchProvider {
  ...
  constructor(
    ...
    this.resourceFetcher = resourceFetcher || new GetMcpResourceFetcher(GETMCP_API_URL);
  ...
  async search(query: string): Promise<MCPServerResponse[]> {
    try {
      ...
      logger.error(`Error searching with Meilisearch: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async ensureDataLoaded(): Promise<void> {
    ...
    try {
      ...
      logger.error(`Error loading data: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
}
