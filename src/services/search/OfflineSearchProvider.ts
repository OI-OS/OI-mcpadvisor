...
export class OfflineSearchProvider implements SearchProvider {
  ...
  constructor(config: OfflineSearchProviderConfig = {}) {
    ...
    this.vectorEngine = new EnhancedMemoryVectorEngine(this.config.fallbackDataPath);
    ...
    try {
      ...
      if (containsKeywords) {
        ...
        if (textMatchResults.length > 0) {
          // 再执行向量搜索（不阻塞返回）
          this.vectorSearch(query).then(vectorResults => {
            logger.debug(`Background vector search found ${vectorResults.length} results`);
          }).catch(error => {
            logger.error(`Background vector search error: ${error instanceof Error ? error.message : String(error)}`);
          });
          
          logger.debug(`Prioritized text match found ${textMatchResults.length} results`);
      ...
      logger.error(`Offline search error: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private async vectorSearch(query: string): Promise<MCPServerResponse[]> {
    try {
      ...
      logger.error(`Vector search error: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private checkQueryForKeywords(query: string): boolean {
    ...
    const priorityKeywords = [
      '小红书', 'xiaohongshu', 'rednote', '热点', '社交媒体',
      '微博', 'weibo', '知乎', 'zhihu'
    ];
    
    return priorityKeywords.some(keyword => queryLower.includes(keyword.toLowerCase()));
  }
  ...
  private async textSearch(query: string): Promise<MCPServerResponse[]> {
    try {
      ...
      for (const result of results) {
        ...
        const descMatch = this.calculateTextSimilarity(query, result.description);
        const categoryMatch = Array.isArray(result.categories) ? 
          Math.max(...result.categories.map((cat: string) => this.calculateTextSimilarity(query, cat))) : 0;
        const tagMatch = Array.isArray(result.tags) ? 
          Math.max(...result.tags.map((tag: string) => this.calculateTextSimilarity(query, tag))) : 0;
        
        // 综合分数 (标题权重高)
        let similarity = titleMatch * 0.5 + descMatch * 0.3 + Math.max(categoryMatch, tagMatch) * 0.2;
      ...
      // 过滤相似度低于阈值的结果
      return results.filter(result => (result.similarity || 0) >= this.config.minSimilarity!);
    } catch (error) {
      logger.error(`Text search error: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private isRedNoteRelated(result: MCPServerResponse): boolean {
    const redNoteKeywords = [
      '小红书', 'xiaohongshu', 'rednote', 'redbook', 'red note'
    ];
    
    // 检查标题
    if (redNoteKeywords.some(keyword => 
    ...
    // 检查分类
    if (Array.isArray(result.categories) && result.categories.some(category => 
    ...
    // 检查标签
    if (Array.isArray(result.tags) && result.tags.some(tag => 
      redNoteKeywords.some(keyword => 
  ...
  private mergeResults(
    ...
    for (const result of vectorResults) {
      if (result.id) {
        // 应用向量搜索权重
        const weightedSimilarity = (result.similarity || 0) * (this.config.vectorSearchWeight || 0.3);
    ...
    for (const result of textResults) {
      if (result.id) {
        ...
        if (existing) {
          // 如果已存在，合并相似度分数
          const textWeightedSimilarity = (result.similarity || 0) * (this.config.textMatchWeight || 0.7);
