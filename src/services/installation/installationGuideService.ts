...
export class InstallationGuideService {
   ...
   * @returns 安装指南内容
   */
  public async generateInstallationGuide(githubUrl: string, mcpName: string): Promise<string> {
    try {
   ...
   * @returns 格式化的安装指南
   */
  private formatGuideForAgentLLM(readmeContent: string, mcpName: string, githubUrl: string): string {
    // 提取安装相关部分
  ...
  private extractInstallationSection(readmeContent: string): string | null {
    ...
    const installationKeywords = [
      'installation', '安装', 'setup', 'getting started', 'quick start', 
      '快速开始', '使用方法', 'usage', 'deploy', '部署'
    ];
    ...
    if (bestMatch) {
      ...
      return this.extractSectionContent(readmeContent, bestMatch.heading, bestMatch.index);
    }
