...
export class EnhancedMemoryVectorEngine implements IWritableVectorSearchEngine {
  ...
  async search(
    ...
    try {
      ...
      // 执行向量搜索
      const vectorResults = await this.memoryEngine.search(normalizedQueryVector, limit * 2);
  ...
  private async ensureFallbackDataLoaded(): Promise<void> {
    ...
    if (!this.fallbackDataLoaded || 
      ...
      try {
        ...
        const fallbackEntries = await this.offlineLoader.loadFallbackDataWithEmbeddings();
        ...
        if (!this.fallbackDataLoaded) {
          for (const entry of fallbackEntries) {
            ...
            await this.memoryEngine.addEntry(entry.id, entry.vector, fallbackData);
          }
          this.fallbackDataLoaded = true;
          logger.info(`Added ${fallbackEntries.length} fallback entries to memory engine`);
        }
   ...
   * 基于文本执行搜索
   */
  private async textSearch(query: string, limit: number): Promise<MCPServerResponse[]> {
    try {
      // 获取所有结果（通过内存引擎搜索，使用空向量）
      const allResults = await this.memoryEngine.search(new Array(384).fill(0), 1000);
      ...
      logger.error(`Error in text search: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private mergeResults(
    ...
    textResults.forEach(result => {
      ...
      if (resultMap.has(key)) {
        ...
        resultMap.set(key, {
          ...existing,
          similarity: (existing.similarity || 0) + (result.similarity || 0) * 0.3
        });
      } else {
        // 如果不存在，添加新条目
        resultMap.set(key, {
  ...
  private applyFilters(
    ...
    if (options.minSimilarity !== undefined) {
      filtered = filtered.filter(result => 
        (result.similarity || 0) >= (options.minSimilarity || 0)
      );
    }
    ...
    if (options.categories && options.categories.length > 0) {
      filtered = filtered.filter(result => {
        ...
        return options.categories!.some(category => 
          Array.isArray(result.categories) && result.categories.some((c: string) => 
    ...
    if (options.tags && options.tags.length > 0) {
      filtered = filtered.filter(result => {
        ...
        return options.tags!.some(tag => 
          Array.isArray(result.tags) && result.tags.some((t: string) => 
