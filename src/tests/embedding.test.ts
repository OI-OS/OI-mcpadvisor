...
describe('嵌入工具测试', () => {
  ...
  describe('提供者切换', () => {
    test('应能在提供者之间切换', () => {
      // 切换到简单提供者
      useSimpleEmbeddingProvider();
      expect(logger.info).toHaveBeenCalledWith('Switched to SimpleEmbeddingProvider');
      
      // 切换到 Universal Sentence Encoder 提供者
      useUniversalSentenceEncoder();
      expect(logger.info).toHaveBeenCalledWith('Switched to UniversalSentenceEncoderProvider');
    });
  });
});
