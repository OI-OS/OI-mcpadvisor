...
class UniversalSentenceEncoderProvider implements EmbeddingProvider {
   ...
   * 批量生成文本嵌入
   */
  async generateEmbeddings(texts: string[], dimensions?: number): Promise<number[][]> {
    try {
      // 确保模型已加载
      await this.ensureModelLoaded();
...
class SimpleEmbeddingProvider implements EmbeddingProvider {
   ...
   * 生成单个文本的嵌入
   */
  async generateEmbedding(text: string, dimensions: number = DEFAULT_DIMENSIONS): Promise<number[]> {
    if (!text) {
   ...
   * 批量生成文本嵌入
   */
  async generateEmbeddings(texts: string[], dimensions: number = DEFAULT_DIMENSIONS): Promise<number[][]> {
   ...
   * 使用简单的算法从文本生成向量
   */
  protected generateVectorFromText(text: string, vector: number[], dimensions: number): number[] {
    const result = [...vector];
   ...
   * 添加n-gram特征
   */
  protected addNgramFeatures(text: string, vector: number[], dimensions: number): void {
    // 处理2-gram和3-gram
    for (let n = 2; n <= 3; n++) {
...
const generateCacheKey = (text: string, dimensions: number): string => {
  return `${text}_${dimensions}_${currentProvider.getName()}`;
};
 ...
 * @returns 归一化的向量表示
 */
export const getTextEmbedding = async (text: string, dimensions: number = DEFAULT_DIMENSIONS): Promise<number[]> => {
  if (!text) {
    return createZeroVector(dimensions);
  }
 ...
 * @returns 归一化的向量表示数组
 */
export const getTextEmbeddings = async (texts: string[], dimensions: number = DEFAULT_DIMENSIONS): Promise<number[][]> => {
  if (!texts || texts.length === 0) {
    return [];
  }
